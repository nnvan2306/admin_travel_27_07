...
export default function HotelPage() {
    ...
    const [actionType, setActionType] = useState<
        "disable" | "enable" | "force-delete" | null
    >(null);
    ...
    const fetchHotels = async () => {
        ...
        try {
            ...
            const updated = res.data
                .filter(
                    (item: any) =>
                        role !== "staff" || item.is_deleted === "active"
                )
                .map((item: any) => ({
    ...
    const handleCreateOrUpdate = async () => {
        try {
            ...
            if (
                values.is_available !== undefined &&
                values.is_available !== null
            ) {
                formData.append(
                    "is_available",
                    values.is_available ? "1" : "0"
                );
            }
            ...
            if (editingHotel) {
                await API.post(
                    `/hotels/${editingHotel.hotel_id}?_method=PUT`,
                    formData,
                    config
                );
            ...
            if (axiosError.response?.status === 422) {
                ...
                const errorMessages = (
                    error as { errorFields: FormError[] }
                ).errorFields
    ...
    const columns: ColumnsType<HotelType> = [
        ...
        { title: "Loại phòng", dataIndex: "room_type", key: "room_type" },
        {
            ...
            key: "price",
            render: (price) =>
                price ? `${price.toLocaleString()} VNĐ` : "Chưa xác định",
        },
        {
            title: "Hình ảnh",
            dataIndex: "image",
        ...
        if (role === "admin") {
            ...
            actions.push({
                ...
                onClick: async () => {
                    try {
                        await API.post(
                            `/hotels/${record.hotel_id}/soft-delete`
                        );
                    ...
                    <Form.Item
                        ...
                        rules={[
                            { required: false },
                            {
                                max: 100,
                                message:
                                    "Loại phòng không được vượt quá 100 ký tự",
                            },
                        ]}
                    >
                    ...
                    <Form.Item
                        ...
                        rules={[
                            { required: false },
                            {
                                max: 100,
                                message:
                                    "Loại giường không được vượt quá 100 ký tự",
                            },
                        ]}
                    >
                        ...
                        >
                            <Upload
                                beforeUpload={(file) => {
                                    ...
                                    if (!isImage) {
                                        notifyError(
                                            "Vui lòng chọn một tệp hình ảnh hợp lệ (JPEG, PNG, GIF, v.v.)"
                                        );
                                        return false;
                                    }
