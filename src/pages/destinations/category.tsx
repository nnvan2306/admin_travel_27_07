...
export default function DestinationCategory() {
    ...
    const handleCreateOrUpdate = async () => {
        try {
            ...
            if (editingCategory) {
                await API.post(
                    `/destination-categories/${editingCategory.id}?_method=PUT`,
                    formData,
                    {
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            if (actionType === "force-delete") {
                ...
                await API.post(
                    `/destination-categories/${selectedId}/soft-delete`,
                    {
                        is_deleted:
                            actionType === "disable" ? "inactive" : "active",
                    }
                );
                notifySuccess(
                    actionType === "disable"
    ...
    const handleEdit = (record: CategoryType) => {
        if (record.is_deleted === "inactive") {
            notifyError(
                "Danh mục này đang bị vô hiệu hóa, không thể chỉnh sửa."
            );
            return;
        }
